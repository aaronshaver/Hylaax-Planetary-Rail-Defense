...
if(window.__HYLAAX_TEST__){
  window.__HYLAAX_TEST_API__={
    ...
    battlefieldSprites,terrainSprite,unitSprite,trainSprite,hiveSprite,depositSprite,createEnemyPathSearch,advanceEnemyPathSearch,processTerrainChunkQueue,
    sounds,
    reset({mapSeed=123456789}={}){
      ...
      terrainCacheSeed=null;terrainCache=new Map();terrainReachabilitySeed=null;terrainReachabilityCache=new Map();terrainRevision=0;terrainChunkSignature="";simulationAccumulator=0;selectionCache="";selectionCacheKey="";selectionInteractionActive=false;selectionRefreshPending=false;selectionRefreshPendingForce=false;zoomGestureActive=false;zoomRenderPending=false;panRenderPending=false;zoomSettleTimer=null;activeRenderBounds=null;invalidateStaticWorldLayers();remindersOpen=false;sounds.enabled=false;
...
function runFrame(now=Date.now()){
  ...
  // Retain simulation debt, but leave room to present input after a long frame.
  const ticks=advanceSimulation((now-lastWallTime)/1000,4),cameraChanged=zoomRenderPending||panRenderPending;lastWallTime=now;zoomRenderPending=false;panRenderPending=false;if(ticks>0||screenShakeActive()||cameraChanged)render();
}
