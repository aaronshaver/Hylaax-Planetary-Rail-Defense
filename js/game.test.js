...
const rootFile=filename=>path.join(PROJECT_ROOT,filename);
...
describe("game bootstrap", () => {
  test("all feature modules load before the entry point", () => {
    ...
    const scriptSources=[...html.matchAll(/<script src="([^"]+\.js)"><\/script>/g)].map(match=>match[1]);
    assert.ok(scriptSources.every(source=>source.startsWith("js/")));
    const gameScripts = scriptSources
  ...
  test("the lower-left Debug button has no FPS widget",()=>{
    ...
    const css=fs.readFileSync(rootFile("styles.css"),"utf8");assert.match(css,/\.bottom-debug-row \{[^}]*left: 5px;[^}]*bottom: 5px;/);
  });
  ...
  test("the feedback contact is anchored to the bottom-right",()=>{
    const css=fs.readFileSync(rootFile("styles.css"),"utf8");
  ...
  test("Center Map on Base is a small button below the upper-left HUD instead of an Action",()=>{
    const html=fs.readFileSync(rootFile("index.html"),"utf8"),css=fs.readFileSync(rootFile("styles.css"),"utf8"),actions=html.match(/<div class="tool-grid"[^>]*>[\s\S]*?<\/div>\s*<\/section>/)?.[0]||"",topLeft=html.match(/<div class="top-left-stack">[\s\S]*?<\/button>\s*<\/div>/)?.[0]||"";
  ...
  test("Restart Tutorial appears before the tutorial text with a flat back icon",()=>{
    const html=fs.readFileSync(rootFile("index.html"),"utf8"),prompt=html.match(/<div id="tutorialPrompt"[\s\S]*?<\/div>\s*<div id="gameOver"/)?.[0]||"";
  ...
  test("legacy three-hex Track construction rules are gone",()=>{
    const sources=["rail.js","trains.js","interface.js"].map(file=>fs.readFileSync(path.join(__dirname,file),"utf8")).concat(fs.readFileSync(rootFile("index.html"),"utf8")).join("\n");
  ...
  test("the Actions panel exposes every tool without number-key shortcuts",()=>{
    const html=fs.readFileSync(rootFile("index.html"),"utf8"),game=fs.readFileSync(path.join(__dirname,"game.js"),"utf8");
  ...
  test("the salvage confirmation shows its title only once",()=>{
    const html=fs.readFileSync(rootFile("index.html"),"utf8"),dialog=html.match(/<div id="confirmDialog"[\s\S]*?<\/div>\s*<\/div>/)?.[0]||"";
  ...
  test("the Actions pane uses three ordered groups without numbered shortcut badges",()=>{
    const html=fs.readFileSync(rootFile("index.html"),"utf8"),css=fs.readFileSync(rootFile("styles.css"),"utf8"),actions=html.match(/<div class="tool-grid"[\s\S]*?<\/div>\s*<\/section>/)?.[0]||"";
