...
function selectionHtml(){
  ...
  if(selected.wagons){const adding=state.mode==="schedule"&&state.scheduleTrainId===selected.id,hasStops=(selected.schedule?.length||0)>0,canAdd=!selected.scheduleComplete&&trainStopped(selected)&&!adding,minimumStops=scheduleMinimumStops(),title=`${selected.name}: ${selectedTrainPartLabel(selected)}`,combatNote=selected.trainType==="combat"?`<div class="selection-subtitle">A mobile turret train · Shot range 6 hexes · 1 damage every 1 second(s) · Restocks its fuel Energy and shot Energy only at Base</div>`:"",primary=adding?button("finish-schedule","Done Adding","btn-command","Finishes the schedule and automatically loops it back to Stop 1."):button("add-schedule","Add Schedule",canAdd?"btn-command":"btn-quiet",`Add ${minimumStops} to 9 Track or Destroyed Track Stops. The Train automatically loops back to Stop 1 when you click Done Adding.`,!canAdd),undo=adding?button("undo-last-stop","Undo Last Stop",hasStops?"btn-command":"btn-quiet","Removes the most recently added Stop.",!hasStops):"",actions=state.gameOver?"":`<div class="panel-actions">${primary}${undo}${button("clear-schedule","Clear Schedule",hasStops||adding?"btn-danger":"btn-quiet","Clears every Stop and stops the Train; while paused, it settles immediately onto the nearest Track hex.",!hasStops&&!adding)}</div>`;return `<div class="selection-title"><h2>${title}</h2></div>${combatNote}${hpBlock(selected)}<div class="status-bar energy"><span style="width:${selected.fuel/selected.maxFuel*100}%"></span></div><div class="status-caption"><span>ENERGY FOR FUEL</span><span>${selected.fuel} / ${selected.maxFuel}</span></div>${cargoHtml(selected)}${actions}`;}
  if(selected.type==="turret")return `<div class="selection-title"><h2>Turret</h2></div><div class="selection-subtitle">Range ${displayStat(turretRange())} hexes · Shoots every ${displayStat(turretFireInterval())} second(s) for ${displayStat(turretDamage())} damage · Instantly refills when a stopped Build/Mine Train Locomotive is adjacent</div>${hpBlock(selected)}${energyBlock(selected,"ENERGY FOR SHOTS")}`;
  if(selected.type==="artillery")return `<div class="selection-title"><h2>Artillery</h2></div><div class="selection-subtitle">Targets Hives only · Range ${displayStat(artilleryRange())} hexes · Shoot every ${displayStat(artilleryFireInterval())} seconds · Uses 10 Energy per shot · ${displayStat(artilleryDamageAtDistance(0))} center damage + ${displayStat(artilleryDamageAtDistance(1))} adjacent-ring damage + ${displayStat(artilleryDamageAtDistance(2))} outer-ring damage per hex · Creeps can take splash damage · No friendly fire</div>${hpBlock(selected)}${energyBlock(selected,"ENERGY FOR SHOTS")}`;
  ...
  if(selected.type==="wall")return `<div class="selection-title"><h2>Wall</h2></div><div class="selection-subtitle">A Train at a Stop instantly repairs damaged Walls within 5 hexes</div>${hpBlock(selected)}`;
