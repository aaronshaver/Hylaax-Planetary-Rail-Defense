...
function selectionHtml(){
  ...
  if(selected.type==="artillery")return `<div class="selection-title"><h2>Artillery</h2></div><div class="selection-subtitle">Targets Hives only · Range ${displayStat(artilleryRange())} hexes · Lobs a shell every ${displayStat(artilleryFireInterval())} seconds · 10 Energy per shot · ${displayStat(artilleryDamage(true))} center damage + ${displayStat(artilleryDamage(false))} damage in each adjacent splash hex · Creeps can take splash damage · No friendly fire</div>${hpBlock(selected)}${energyBlock(selected)}`;
  ...
  if(selected.type==="research"){
    const actions=state.gameOver?"":`<div class="panel-actions research-actions">${RESEARCH_UPGRADES.map(upgrade=>button(`research-${upgrade.key}`,upgrade.label,"btn-quiet",`• ${upgrade.description}&#10;• Costs ${RESEARCH_UPGRADE_COST} (R)esearch points.`,state.researchPoints+1e-9<RESEARCH_UPGRADE_COST)).join("")}</div>`;
    return `<div class="selection-title"><h2>Research</h2></div><div class="selection-subtitle">${displayStat(researchRate())} Research points per second · Upgrades apply instantly and can be researched indefinitely</div>${hpBlock(selected)}<div class="research-points-summary">Research Points <strong>${Math.floor(state.researchPoints)}</strong></div>${actions}`;
  }
