...
function neutralizerStorageHtml(building){return `<div class="cargo-list"><div class="cargo-row material" data-neutralizer-resource="material"><i></i><span>Stored construction material</span><strong>${Math.floor(building.material)} / ${building.maxMaterial}</strong></div><div class="cargo-row energy" data-neutralizer-resource="energy"><i></i><span>Stored energy</span><strong>${Math.floor(building.energy)} / ${building.maxEnergy}</strong></div></div>`;}
...
function selectionHtml(){
  ...
  if(selected.type==="base"){
    ...
    const builderTip=`• Costs ${COSTS.train.material} C, ${COSTS.train.energy} E&#10;• Places one locomotive with an empty construction material supply and an empty energy supply.&#10;• Mines resources, repairs buildings, rebuilds destroyed track, supplies turrets and artillery with shot energy, and supplies neutralizer buildings with construction material and energy${builderUnavailableTip}`;
    const combatTip=`• Costs ${COSTS.combatTrain.material} C, ${COSTS.combatTrain.energy} E&#10;• Places one locomotive with one energy supply holding 10 energy.&#10;• Automatically fires at hives and creeps within 6 hexes, including while moving.&#10;• Must normally be refueled and have shot energy resupplied from the base building (cannot be supplied by mines); another train can provide emergency fuel when it has no fuel energy remaining${combatUnavailableTip}`;
    ...
    const unloadingNote=destroyed?"":`<div class="selection-subtitle">Trains will only fill each base resource to 110 units</div>`;
    return `<div class="selection-title"><h2>${destroyed?"Destroyed base building":"Base building"}</h2></div>${destroyed?`<div class="selection-subtitle">Destroyed · The base building cannot be rebuilt or salvaged</div>`:""}${hpBlock(selected)}${baseInventoryHtml()}${unloadingNote}${state.gameOver?"":deployNote}${actions}`;
  }
  if(selected.wagons){const adding=state.mode==="schedule"&&state.scheduleTrainId===selected.id,hasStops=(selected.schedule?.length||0)>0,draft=adding&&state.scheduleDraft?.trainId===selected.id?state.scheduleDraft:null,newStops=adding?selected.schedule.length-(draft?.originalSchedule.length||0):0,canAdd=trainStopped(selected)&&!adding&&selected.schedule.length<9,minimumStops=scheduleMinimumStops(),title=`${selected.name}: ${selectedTrainPartLabel(selected)}`,combatNote=selected.trainType==="combat"?`<div class="selection-subtitle">A mobile turret train · Shot range 6 hexes · 1 damage every 1 second(s) · Cannot shoot over mountains; can shoot across water, trees, and walls · Restocks its fuel energy and shot energy at the base building · Another train can provide emergency fuel when it has no fuel energy remaining</div>`:"",addHelp=hasStops?`Add more track or destroyed track stops to the existing schedule, up to 9 total. Existing stops are preserved, and Undo last stop applies only to stops added during this session.`:`Add ${minimumStops} to 9 track or destroyed track stops. The train automatically loops back to stop 1 when you click Done adding.`,primary=adding?button("finish-schedule","Done adding","btn-command","Finishes the schedule and automatically loops back to stop 1."):button("add-schedule","Add schedule",canAdd?"btn-command":"btn-quiet",addHelp,!canAdd),undo=adding?button("undo-last-stop","Undo last stop",newStops>0?"btn-command":"btn-quiet","Removes the most recently added stop from this adding session.",newStops<=0):"",actions=state.gameOver?"":`<div class="panel-actions">${primary}${undo}${button("clear-schedule","Clear schedule",hasStops||adding?"btn-danger":"btn-quiet","Clears every stop and stops the train; while paused, it settles immediately onto the nearest track hex.",!hasStops&&!adding)}</div>`;return `<div class="selection-title"><h2>${title}</h2></div>${combatNote}${hpBlock(selected)}<div class="status-bar energy"><span style="width:${selected.fuel/selected.maxFuel*100}%"></span></div><div class="status-caption"><span>Energy for fuel</span><span>${selected.fuel} / ${selected.maxFuel}</span></div>${cargoHtml(selected)}${actions}`;}
  if(selected.type==="turret")return `<div class="selection-title"><h2>Turret</h2></div><div class="selection-subtitle">Range ${displayStat(turretRange())} hexes · Shoots every ${displayStat(turretFireInterval())} second(s) for ${displayStat(turretDamage())} damage · Cannot shoot over mountains; can shoot across water, trees, and walls · Instantly refills when a build/mine train is stopped at an adjacent non-destroyed train stop</div>${hpBlock(selected)}${energyBlock(selected,"Energy for shots")}`;
  ...
  if(selected.type==="gate")return `<div class="selection-title"><h2>Gate</h2></div><div class="selection-subtitle">Same cost and hit points as a wall · Wall hit-point research also applies · Neutralizer ally units can pass through · Creeps cannot pass through · A train stopped at a non-destroyed train stop repairs gates within ${WALL_SERVICE_RANGE} hexes</div>${hpBlock(selected)}`;
  if(selected.type==="neutralizer-building")return `<div class="selection-title"><h2>Neutralizer building</h2></div><div class="selection-subtitle">Produces 1 neutralizer ally every ${displayStat(neutralizerProductionInterval())} second(s) while at least ${NEUTRALIZER_UNIT_MATERIAL_COST} construction material and ${NEUTRALIZER_UNIT_ENERGY_COST} energy are stored · Each produced neutralizer consumes ${NEUTRALIZER_UNIT_MATERIAL_COST} of each resource and has a 1.5-minute lifespan · Supplied by a build/mine train stopped at an adjacent non-destroyed train stop</div>${hpBlock(selected)}${neutralizerStorageHtml(selected)}`;
  ...
  if(selected.type==="hive"){const rate=selected.level,cycle=rate===1?"Produces 1 creep when it appears and once per minute thereafter":"When it appears and once per minute thereafter, has a 1 in "+rate+" chance to produce one Level "+rate+" hive; otherwise produces "+rate+" creeps";return `<div class="selection-title"><h2>Level ${rate} hive</h2></div><div class="selection-subtitle">${cycle} · New hives and creeps appear after a 1–5 second delay</div>${hpBlock(selected)}`;}
  if(selected.type==="enemy")return `<div class="selection-title"><h2>Creep</h2></div><div class="selection-subtitle">1.5-minute lifespan · Can cross water, but moves at half speed when leaving a water hex</div>${hpBlock(selected)}`;
  if(selected.type==="neutralizer")return `<div class="selection-title"><h2>Neutralizer ally</h2></div><div class="selection-subtitle">1.5-minute lifespan · Seeks the nearest creep or hive · Shoots every ${displayStat(neutralizerFireInterval())} second(s) for ${displayStat(neutralizerDamage())} damage · Can cross water at half speed when leaving a water hex · Can pass through gates but not walls, mountains, or trees</div>${hpBlock(selected)}`;
  if(selected.type==="ghost"){const name=selected.objectType==="track"?"track":selected.objectType==="turret"?"turret":selected.objectType==="artillery"?"artillery":selected.objectType==="wall"?"wall":selected.objectType==="gate"?"gate":selected.objectType==="research"?"research building":selected.objectType==="neutralizer-building"?"neutralizer building":`${resourceLabel(selected.resource)} mine`,scheduled=selected.objectType==="track"?scheduleStopAt(selected.q,selected.r):null,title=scheduled?`Destroyed stop ${trainScheduleCode(scheduled.train)}${scheduled.index+1} (${scheduled.train.name})`:`Destroyed ${name}`,mineNode=selected.objectType==="mine"?resourceNodeAt(selected.q,selected.r):null,mineNote=mineNode?.amount>0?" Because this resource node still has resources, only a mine can be built here.":"",rebuildNote=selected.objectType==="track"?`An adjacent stopped train carrying ${REBUILD_COSTS.track} construction material can rebuild destroyed track even when the train is not at a stop.`:`A train stopped at an adjacent non-destroyed train stop and carrying ${REBUILD_COSTS[selected.objectType]} construction material can rebuild the original object automatically.`;return `<div class="selection-title"><h2>${title}</h2></div><div class="selection-subtitle">Destroyed · Any construction normally allowed on this terrain can replace this wreckage directly. ${rebuildNote} Salvage/clear object removes the wreckage without recovering resources.${mineNote}</div>`;}
...
function showLowBaseResourceWarning(){
  if(state.lowBaseResourceWarningShown||state.gameOver||state.elapsed<300||(state.baseMaterial>0&&state.baseEnergy>0))return false;
  state.lowBaseResourceWarningShown=true;state.lowBaseResourceWarningWasPaused=state.paused;state.paused=true;simulationAccumulator=0;
  ui.lowBaseResourceDialog.hidden=false;ui.lowBaseResourceDialog.classList.remove("d-none");updateUI(true);render();setTimeout(()=>ui.lowBaseResourceOkay.focus(),0);return true;
}

function dismissLowBaseResourceWarning(){
  if(ui.lowBaseResourceDialog.hidden)return false;
  ui.lowBaseResourceDialog.hidden=true;ui.lowBaseResourceDialog.classList.add("d-none");state.paused=state.lowBaseResourceWarningWasPaused;lastWallTime=Date.now();simulationAccumulator=0;updateUI(true);render();canvas.focus();return true;
}
...
function updateNeutralizerBuildingSelectionContent(building){
  const query=selector=>selectionContent.querySelector?.(selector)||null;
  const hpFill=query(".status-bar > span");if(hpFill)hpFill.style.width=`${clamp(building.hp/building.maxHp*100,0,100)}%`;
  const hpValue=query(".status-caption span:last-child");if(hpValue)hpValue.textContent=`${Math.ceil(building.hp)} / ${Math.ceil(building.maxHp)}`;
  for(const resource of ["material","energy"]){const value=query(`[data-neutralizer-resource="${resource}"] strong`);if(value)value.textContent=`${Math.floor(building[resource])} / ${building[resource==="material"?"maxMaterial":"maxEnergy"]}`;}
}
...
function updateUI(force=false){
  ...
  if(force||selectionMarkup!==selectionCache){
    ...
    if(selected?.type==="neutralizer-building"&&cacheKey===selectionCacheKey&&selectionCache){updateNeutralizerBuildingSelectionContent(selected);selectionCache=selectionMarkup;return;}
