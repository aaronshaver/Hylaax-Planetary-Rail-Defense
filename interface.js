...
function selectionHtml(){
  ...
  if(selected.type==="base"){
    ...
    return `<div class="selection-title"><h2>Base</h2></div>${hpBlock(selected)}${baseInventoryHtml()}${state.gameOver?"":deployNote}${actions}`;
  }
  ...
  if(selected.type==="ghost"){const name=selected.objectType==="track"?"Track":selected.objectType==="turret"?"Turret":selected.objectType==="artillery"?"Artillery":selected.objectType==="wall"?"Wall":`${resourceLabel(selected.resource)} Mine`,scheduled=selected.objectType==="track"?scheduleStopAt(selected.q,selected.r):null,title=scheduled?`Destroyed Stop ${trainScheduleCode(scheduled.train)}${scheduled.index+1} (${scheduled.train.name})`:`Destroyed ${name}`;return `<div class="selection-title"><h2>${title}</h2></div><div class="selection-subtitle">Stops being a ghost when a locomotive carrying ${REBUILD_COSTS[selected.objectType]} Construction Material stops adjacent to it</div>`;}
