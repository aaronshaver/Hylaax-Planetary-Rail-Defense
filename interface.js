...
function selectionHtml(){
  ...
  if(selected.type==="enemy")return `<div class="selection-title"><h2>Creep</h2></div><div class="selection-subtitle">2-minute lifespan · Can cross water, but moves at half speed when leaving a water hex</div>${hpBlock(selected)}`;
  if(selected.type==="neutralizer")return `<div class="selection-title"><h2>Neutralizer ally</h2></div><div class="selection-subtitle">2-minute lifespan · Seeks the nearest creep or hive · Shoots every ${displayStat(neutralizerFireInterval())} second(s) for ${displayStat(neutralizerDamage())} damage · Can cross water at half speed when leaving a water hex · Can pass through gates but not walls, mountains, or trees</div>${hpBlock(selected)}`;
