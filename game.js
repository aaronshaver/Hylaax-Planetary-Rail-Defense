...
if(window.__HYLAAX_TEST__){
  window.__HYLAAX_TEST_API__={
    constants:{NODE_MIN_CAPACITY,NODE_MAX_CAPACITY,ENERGY_NODE_MIN_CAPACITY,ENERGY_NODE_MAX_CAPACITY,BASE_TRAIN_STOP_SECONDS,TRAIN_ACTIVITY_MESSAGE_SECONDS,INITIAL_HIVE_COUNT,ENEMY_SPEED,CREEP_ATTACK_INTERVAL,CREEP_ATTACK_DAMAGE,SIMULATION_STEP,TRACK_HIT_POINTS,TRAIN_HIT_POINTS,WALL_HIT_POINTS,TURRET_HIT_POINTS,RESEARCH_HIT_POINTS,RESEARCH_UPGRADE_COST,NEUTRALIZER_BUILDING_HIT_POINTS,NEUTRALIZER_BASE_HIT_POINTS,NEUTRALIZER_BASE_DAMAGE,NEUTRALIZER_ATTACK_INTERVAL,NEUTRALIZER_SPEED,NEUTRALIZER_BASE_STORAGE,NEUTRALIZER_PRODUCTION_INTERVAL,NEUTRALIZER_UNIT_MATERIAL_COST,NEUTRALIZER_UNIT_ENERGY_COST,NEUTRALIZER_TARGET_REFRESH_SECONDS,NEUTRALIZER_PATH_SEARCHES_PER_TICK,NEUTRALIZER_PATH_CACHE_SECONDS,TRAIN_LOSS_SHAKE_SECONDS,ENEMY_SPAWN_BUFFER,REPAIR_PAUSE_SECONDS,WALL_SERVICE_RANGE,SALVAGE_BURST_SCALE,TURRET_RANGE,COMBAT_TRAIN_RANGE,ARTILLERY_RANGE,ARTILLERY_HIT_POINTS,ARTILLERY_MAX_ENERGY,ARTILLERY_FIRE_INTERVAL,ARTILLERY_SHELL_FLIGHT_SECONDS,ARTILLERY_BLAST_SECONDS,ARTILLERY_SHOT_ENERGY,ARTILLERY_CENTER_DAMAGE,ARTILLERY_SPLASH_DAMAGE,ARTILLERY_OUTER_SPLASH_DAMAGE,CREEP_HEX_CAPACITY,CREEP_SLOT_RADIUS,CREEP_RENDER_SCALE,BASE_UNLOAD_TARGET,BASE_FOOTPRINT_OFFSETS,HIVE_LEVELS,COSTS,REBUILD_COSTS,BASE_RESOURCE_TYPES,TRAIN_CAR_COLORS,DIRECTIONS,RESEARCH_UPGRADES},
    ...
    hiveUnlockedLevel,nextHiveLevel,hiveExpansionLevel,createHive,hiveHexOpen,hiveReplicationRoll,hiveSpawnCandidates,hiveSpawnDelay,queueHiveSpawn,spawnHiveNear,processHiveSpawnQueue,spawnEnemyAt,spawnEnemyFromHive,creepSpawnDelay,queueCreepBatch,processCreepSpawnQueue,debugAddMaxCreepsAt,playerConstructionAnchors,outsidePlayerConstructionBuffer,encroachingHiveLocation,spawnEncroachingHive,encroachingHiveCount,encroachmentOccurs,updateEncroachingHives,hiveProductionDelay,queueDueHiveProductions,produceHiveOperation,processHiveProductionQueue,updateHives,
    ...
    enemySlotOffset,enemyWorldPosition,reserveEnemySpace,releaseEnemySpace,enemySpaceReservations,enemyHexHasRoom,chooseEnemySpaceSlot,resetEnemyNavigation,rebuildEnemyNavigation,ensureEnemyNavigation,nextEnemyNavigationStep,adjacentEnemyTarget,enemyNavigationStats,findEnemyStep,updateEnemies,
    neutralizerFootprintCandidates,neutralizerCellAvailable,neutralizerPlacementFootprint,neutralizerPreviewFootprint,showNeutralizerGateNotice,dismissNeutralizerGateNotice,buildNeutralizer,neutralizerSpaceReservations,neutralizerCanTraverse,neutralizerTargets,neutralizerTargetLookup,cachedNeutralizerTarget,cachedNeutralizerPathStep,adjacentNeutralizerTarget,neutralizerSpawnLocation,spawnNeutralizerAt,spawnNeutralizer,debugAddMaxNeutralizersAt,updateNeutralizerProduction,neutralizerNextStep,damageNeutralizer,updateNeutralizers,
    updateTrains,updateCombatTrains,fireArtillery,resolveArtilleryImpact,updateProjectiles,updateStructures,burstAt,burst,update,advanceSimulation,
...
document.addEventListener("keydown",e=>{if(remindersOpen){if(e.key==="Escape"||e.key==="Enter")startGame(false);return;}if(!ui.turretEnergyDialog.hidden){if(e.key==="Escape"||e.key==="Enter")dismissTurretEnergyWarning();return;}if(!ui.trackDestroyedDialog.hidden){if(e.key==="Escape"||e.key==="Enter")dismissTrackDestroyedWarning();return;}if(!ui.neutralizerGateDialog.hidden){if(e.key==="Enter")dismissNeutralizerGateNotice();return;}if(!ui.confirmDialog.hidden){if(e.key==="Escape")cancelTrainSalvage();return;}if(e.target.matches("input,textarea"))return;if(e.key>="1"&&e.key<="9"){setMode(["select","track","turret","mine","wall","artillery","salvage","research","gate"][Number(e.key)-1]);}if(e.key==="0")setMode("neutralizer");if(e.key==="Escape")setMode("select");});
...
ui.gameOver.hidden=true;ui.viewFinalStats.hidden=true;ui.confirmDialog.hidden=true;ui.turretEnergyDialog.hidden=true;ui.trackDestroyedDialog.hidden=true;ui.neutralizerGateDialog.hidden=true;resize();initializeTooltips();updateUI(true);showReminders();
